...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let src_start_x_f = f32(dst_x_u) * scale_x;
    let src_start_y_f = f32(dst_y_u) * scale_y;
    let src_end_x_f = f32(dst_x_u + 1u) * scale_x;
    let src_end_y_f = f32(dst_y_u + 1u) * scale_y;
